...
= 3.1.0 =
Downloads now redirect to the file's own URL by default instead of
streaming through your server; add `add_filter( 'wppa_mask_url', '__return_true' );`
to opt back into the old streaming/proxy behavior for large files. Because
a redirect cannot set a `Content-Disposition` filename the way a
streamed response can, a redirected download's filename is whatever the
browser derives from the URL itself, not a name this plugin can force.
Also: hook callbacks that named a 3.0.1 global class (e.g.
`array( 'WP_Publication_Archive', 'save_meta' )`) can no longer be found
and removed with `remove_action()`/`remove_filter()` — every hook this
plugin registers now uses an internal callable instead.
